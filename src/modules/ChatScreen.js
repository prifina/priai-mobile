  ...
  const handleSubmit = async () => {
    ...
    try {
      const result = await openai.createChatCompletion({
        // model: 'gpt-3.5-turbo',
        model: 'gpt-3.5-turbo',
        messages: [].concat([{role: "system", content: initalPrompt}],conversation.slice(conversation.length > 12 ? conversation.length - 12 : 0).map((exchange)=>{
          return {
            role: exchange.speaker === defaultValues.name ? "user" : "assistant",
